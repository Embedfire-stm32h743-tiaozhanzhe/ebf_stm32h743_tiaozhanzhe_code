...
__attribute__ ((at(0x30000000))) uint16_t buffer0[RECBUFFER_SIZE];  /* 数据缓存区1 ，实际占用字节数：RECBUFFER_SIZE*2 */
__attribute__ ((at(0x30004000))) uint16_t buffer1[RECBUFFER_SIZE];  /* 数据缓存区2 ，实际占用字节数：RECBUFFER_SIZE*2 */
...
static void StartRecord(const char *filename)
{
	...
	else   /* MIC输入 */
	{
		...
		wm8978_CfgAudioPath(MIC_LEFT_ON | MIC_RIGHT_ON | ADC_ON, OUT_PATH_OFF | OUT_PATH_OFF);	
